...
html_template += f"""
    ...
    <script>
        ...
        function sendChatMessage() {{
            ...
            .catch(err => {{
                console.log('Error calling Vercel chat API:', err);
                const indicator = document.getElementById('typingIndicator');
                if (indicator) indicator.remove();
                
                if (err.message.includes('Too Many Requests') || err.message.includes('429')) {{
                    appendMessage('⚠️ <strong>Límite de Consultas Excedido:</strong> El asistente ha recibido demasiadas preguntas seguidas bajo el plan gratuito de Gemini. Por favor, espera 15 a 30 segundos para que se libere la cuota e intenta tu pregunta nuevamente.', 'assistant');
                }} else {{
